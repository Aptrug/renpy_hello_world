...
define round_bg        = Circle(125, (80,80,80), (50,50,50), 3)    # Outer circle with border
define orb_active     = Circle(25, (255,215,0), (184,134,11), 2)  # Gold orb (active)
define orb_inactive_img = Circle(25, (102,102,102), (60,60,60), 2) # Grey orb (inactive)
    ...
    fixed:
        ...
        for i, (x, y) in enumerate(orb_positions):
            if i < available_ap:
                ...
                add orb_inactive_img xpos x ypos y:
                    alpha 0.4
                    zoom 0.9
...
label start:
    ...
    menu:
        "Spend AP" if available_ap > 0:
            $ available_ap -= 1; renpy.restart_interaction()
            jump start
        "Gain AP" if available_ap < max_ap:
            $ available_ap += 1; renpy.restart_interaction()
            jump start
        "Next Round":
            $ current_round += 1; available_ap = max_ap; renpy.restart_interaction()
            jump start
        "Exit":
            return
