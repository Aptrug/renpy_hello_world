...
init python:
    ...
    ENEMY_COLOR_BASE = (0x66, 0x00, 0x00)
    ENEMY_COLOR_RANGE = (0x99, 0x33, 0x33)
    HERO_COLOR_BASE = (0x1a, 0x23, 0x7e)
    HERO_COLOR_RANGE = (0x27, 0x46, 0x63)

    # Initialize quickmenu backup if needed
    ...
    def get_orb_positions(num_orbs):
        if num_orbs not in _orb_cache:
            positions = []
            if num_orbs > 0:
                step = 6.283185307179586 / num_orbs
                for i in range(num_orbs):
                    angle = step * i - 1.5707963267948966
...
screen battle_ui():
    ...
    # HP Bars and Round Circle - centered middle
    ...
    hbox:
        ...
        # Enemy HP bar (Red - unified for all enemies)
        ...
        # Hero HP bar (Blue - unified for all allies)
        use hp_bar_section("Heroes", current_hp, max_hp, hero_color, bar_width)

    # Hero party - bottom row
    hbox:
        xalign 0.5
        yalign 0.95
        spacing 20
...
screen hp_bar_section(label, hp_value, max_hp_value, color, width):
    vbox:
        spacing 5
        text label size 24 color "#ffffff"
        ...
        text "[hp_value]%" size 24 color "#ffffff"

# ========================
# Battle Flow
# ========================
label start_battle:
    # Backup quickmenu state
    ...
    "Round [current_round] begins! A wild boss appears!"
    "Heroes prepare for battle!"

    menu:
        "Kenshin attacks!":
            $ enemy_hp = max(0, enemy_hp - 25)
            "Kenshin strikes for 25 damage!"

        "Magic user casts spell!":
            $ enemy_hp = max(0, enemy_hp - 20)
            $ current_hp = min(max_hp, current_hp + 10)
            "Magic deals 20 damage and heals allies for 10!"

        "Rance charges!":
            $ enemy_hp = max(0, enemy_hp - 30)
            $ current_hp = max(0, current_hp - 5)
            "Rance deals heavy damage but takes 5 recoil!"

        "Use AP ability" if available_ap > 0:
            $ available_ap -= 1
            $ enemy_hp = max(0, enemy_hp - 35)
            "Powerful ability deals 35 damage!"

    if enemy_hp > 0:
        "Boss counterattacks!"
        $ current_hp = max(0, current_hp - 20)
        "Heroes take 20 damage!"

    if enemy_hp <= 0:
        "Victory! The boss is defeated!"
        jump battle_victory
    elif current_hp <= 0:
        "Defeat! The heroes fall..."
        jump battle_defeat
    else:
        jump start_battle
